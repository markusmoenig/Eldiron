...
- **text**: multiline string content; placeholders like `{PLAYER.CLASS}`, `{PLAYER.RACE}`, `{PLAYER.STR}`, `{PLAYER.DEX}`, `{PLAYER.FUNDS}`, `{PLAYER.LEVEL}`, `{PLAYER.EXP}`, `{PLAYER.ATTACK}`, and `{PLAYER.ARMOR}` are replaced at runtime.
