...
- `ui.spellbook` and `ui.actions` are navigation commands and normally belong in the left group. They are not executable ruleset actions. The modern player HUD normally exposes `ui.spellbook`; add `ui.actions` when a complete generic ruleset catalogue is useful, such as a compact or hardcoded UI.
- `[ui.spellbook]` configures the Toolbar's built-in Spellbook. `columns` and `rows` set the visible page capacity, `cell_size` sets square entry dimensions, `spacing` and `padding` control the grid, `icon_inset` controls icon size, and `show_names` toggles labels. When the spell count exceeds `rows * columns`, the title bar provides page controls. `[ui.actions]` accepts the same settings for the generic Actions catalogue.
...
- The built-in panel is the zero-setup path: a screen only needs its action-bar widget and a `ui.spellbook` command. A screen sector with `role = "spellbook"` overrides the Toolbar configuration and authored panel rectangle. This lets a game replace the default layout and artwork without changing commands or ruleset spell data.
- Navigation-command artwork is supplied by `[ui.commands.<name>]` in the ruleset. The official artist-editable placeholders are `assets/icons/ui_spellbook/on/0.png` and `assets/icons/ui_actions/on/0.png`.
...
`ui.spellbook` opens a draggable panel filtered to the Spells group. Its default layout comes from `[ui.spellbook]` on the action bar. To replace it for one screen, add a sector with settings such as:
...
The sector's rectangle and responsive `[layout]` determine the custom panel's placement and size. Frame and slot tiles use nine-slice scaling. The Spellbook still reads spells, icons, descriptions, requirements, and targeting behavior from the active ruleset.

Click an action to select it for normal use. Drag an action onto any button with `command_slot = "<group>.<index>"` to assign it to that quick slot. For touch or non-drag interfaces, click **Assign**, click an action, and then click the destination quick slot.
