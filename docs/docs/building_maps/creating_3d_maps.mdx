...
Palette colors keep their own material preset and finish metadata, so changing a palette entry's material updates 3D surfaces that use that palette index.
