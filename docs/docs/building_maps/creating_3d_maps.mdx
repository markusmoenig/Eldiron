...
In 3D views, the HUD exposes two geometry modes:

- `GEOM`: edit world geometry directly.
- `DETAIL`: edit the profile of the clicked surface directly in the 3D view.

Use `GEOM` for layout, extrusions, roofs, and structural work. Use `DETAIL` for holes, windows, gates, recesses, reliefs, and other surface-local detail.
...
Let’s cut a hole in a wall for a passage. Switch to `DETAIL` mode and click the wall you want to edit.

![3D Tutorial Step4](3d_tutorial4.png)

After selecting the wall in `DETAIL` mode, the normal 3D tools work on that surface profile.

![3D Tutorial Step5](3d_tutorial5.png)

The turquoise outline shows the boundaries of the active surface. Everything you draw in `DETAIL` mode is stored in that surface profile.
...
Instead of just using a **surface profile** as a hole, you can also apply the [Recess](/docs/creator/actions/#recess) and [Relief](/docs/creator/actions/#relief) actions to create a recess and relief of a given depth or height. These actions appear when `DETAIL` mode is active and a profile sector is selected. You can drag and drop tiles for the sides and cap to the given action.
...
Of course you can further decorate roof surfaces or floors in `DETAIL` mode with profiles (recesses / reliefs) like we did with the side wall.
