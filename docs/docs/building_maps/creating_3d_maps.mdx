...
Create Cutout uses the actual closed loop shapes. It rebuilds the front face and the opposite face around the openings, then creates reveal faces through the object thickness. The guide loops remain selected after the cutout, so they can be moved, duplicated, reused, or deleted explicitly. Surface guides can also be reselected on the rebuilt surface ring, which makes repeated custom windows, vents, floor holes, and non-rectangular openings possible without redrawing the original shape.
