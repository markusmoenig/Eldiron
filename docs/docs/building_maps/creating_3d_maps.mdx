...
Use **Edit Face Texture** on selected faces, or on a selected object, to adjust texture offset, scale, and rotation. Texture scale follows the old 3D path convention: larger scale values make the source cover a larger area, while smaller values repeat it more tightly.
