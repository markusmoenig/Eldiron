...
- **Create Pattern** stamps editable guide patterns such as discs, triangles, quads, lines, staggered brick grids, regular tile grids, or alternating comma sequences onto selected faces. The guides remain normal surface detail, so they can be inspected, edited, and then committed with Create Face, Create Cutout, Create Ridge, or Create Groove.
