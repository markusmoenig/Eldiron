...
The **Prefab Tool** opens the **Prefabs** dock for browsing and placing linked assets. Select a project Prefab, then use the top-right **Edit / Maximize Dock** control (`Cmd/Ctrl + [`) to open the isolated Prefab editor. There, the normal Object, Face, Edge, Vertex, camera, undo, and 3D Paint tools edit the shared source. The lower editor panel contains the hierarchical Prefab / part / Geometry Object tree, part properties, pivots, and Prefab-specific controls. Use **Restore Dock** (`Cmd/Ctrl + ]`) to return to the normal split layout.
