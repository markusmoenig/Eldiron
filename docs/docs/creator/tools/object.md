...
- **Edit Geometry**: Sets exact object bounds, object visibility, mesh-collision solidity, an optional group label, and 3D area metadata. Solid objects feed their walkable face planes and vertical side barriers into mesh collision. **Area is opt-in**: a new object keeps its label ("Box", "Column 1", …) but is not a named place until you tick *Area*, so primitives and block parts stay out of the region tree's area list and cannot be used as script destinations. A ticked object can be used as a sector-style script destination, and **Hide in Iso** fades the object out while the player is inside its area in isometric gameplay.
...
The contextual shortcut overlay at the bottom of the geometry editor updates after each 3D selection change and shows the shortcuts currently available for the selected object, face, edge, or vertex. It is hidden when the active tool has no contextual guidance to display.
