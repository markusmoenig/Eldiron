...
Eldiron v0.9.1 is a workflow-focused update that makes projects easier to debug, balance, localize, and polish. The biggest additions are the new global rules, localization, and audio FX systems, along with stronger runtime feedback while iterating on gameplay.
...
Creator now gives clearer realtime feedback while a game is running. Script execution and values are easier to inspect, making assignments and conditions reveal much more of what is going on internally.

This makes gameplay scripting feel much less like guesswork and much more like working with a live system you can actually inspect while it runs.
...
## Grid Movement And Input Mapping

This release also expands player input mapping with two new grid-based modes for `set_player_camera(...)`:

- `2d_grid`
- `firstp_grid`

These modes keep movement smooth, but interpret each action as a single tile / world-unit step. `firstp_grid` also supports 90-degree turning and tile-based sidestepping via `strafe_left` and `strafe_right`.

Grid movement now also works correctly with collision-driven interactions such as doors and gates, so item handlers that react to bump contact continue to behave as expected in both 2D and first-person grid play.
