...
fn extrude_selected_geometry_faces(map: &mut Map, amount: f32) -> bool {
    if map.selected_geometry_faces.is_empty() {
        return false;
    }
...
fn subdivide_selected_geometry_faces(map: &mut Map) -> bool {
    if map.selected_geometry_faces.is_empty() {
        return false;
    }
    ...
    for object in &mut map.geometry_objects {
        ...
        for face_index in face_indices {
            ...
            object.faces[face_index] = make_face(vec![a, ab, center, da]);
            new_selected_faces.push((object.id, face_index));

            object.faces.push(make_face(vec![ab, b, bc, center]));

            object.faces.push(make_face(vec![center, bc, c, cd]));

            object.faces.push(make_face(vec![da, center, cd, d]));
...
impl Tool for GeometryTool {
    ...
    fn map_event(
        ...
        match map_event {
            ...
            MapKey(key) => {
                let step = 1.0 / map.subdivisions.max(1.0);
                if matches!(key, 'e' | 'E') && !map.selected_geometry_faces.is_empty() {
                    let old_map = map.clone();
                    if !extrude_selected_geometry_faces(map, step) {
                        return None;
                    }
                    sanitize_geometry_selection(map);
                ...
                if matches!(key, 'x' | 'X') && !map.selected_geometry_faces.is_empty() {
                    let old_map = map.clone();
