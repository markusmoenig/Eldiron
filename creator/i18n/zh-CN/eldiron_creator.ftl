...
action_cut_hole = 切割孔洞
action_cut_hole_desc = 根据选中的内部区域在宿主区域上创建一个轮廓切口，并保留一个隐藏的已链接几何句柄以供后续建模使用。
...
action_build_shaft = 构建竖井
action_build_shaft_desc = 从隐藏的切口句柄区域生成真实的竖井墙体，并可选生成底盖。
action_build_stairs = 构建楼梯
action_build_stairs_desc = 在两条选中的线段之间生成真实的直线楼梯。
...
action_window_desc = 在选中的轮廓区域创建静态窗户（窗框 + 玻璃）。
