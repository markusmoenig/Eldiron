...
status_action_edit_sector_terrain = 设置区域的地形角色。
...
status_action_edit_sector_item = 设置将在该区域实例化的物品。
status_action_edit_sector_visible = 区域几何的可见状态。
...
status_action_edit_vertex_terrain_control = 将顶点设为地形控制点。
status_action_edit_vertex_terrain_smoothness = 地形影响半径（越大 = 范围越广，坡度更平缓）。
status_action_edit_vertex_tile = 在顶点位置的公告板图块。
...
status_action_editing_slice_pos = 2D 编辑切片的垂直位置。
...
status_action_gate_door_inset = 门 / 门洞的内缩距离。
status_action_gate_door_tile = 门 / 门洞使用的图块。
status_action_gate_door_repeat_mode = 门 / 门洞的重复模式。
...
status_action_apply_tile_mode = 图块的重复模式。
## Menubar
...
action_edit_sector_terrain = 地形角色
action_edit_sector_terrain_none = 无
action_edit_sector_terrain_exclude = 排除
...
action_edit_sector_item = 物品名称
action_edit_sector_visible = 可见
...
action_edit_vertex_terrain_control = 地形
action_edit_vertex_terrain_smoothness = 平滑度
