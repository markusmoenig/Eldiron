...
action_cut_hole = Cut Hole
action_cut_hole_desc = Creates a profile cutout on the selected host sector from the selected inner sector and keeps a hidden linked handle for follow-up geometry work.
...
action_build_procedural_help = Builds the current region's [procedural] configuration into editable map geometry. Use mode = "2d" for tile maps or mode = "3d" for the reserved 3D dungeon path.
action_build_room = Build Room
action_build_room_desc = Builds a room behind the selected wall sector and opens the entry by default.
action_build_shaft = Build Shaft
action_build_shaft_desc = Builds real shaft walls and an optional bottom cap from a hidden cutout handle sector.
action_build_stairs = Build Stairs
action_build_stairs_desc = Builds straight physical stairs between two selected linedefs.
...
action_window_desc = Creates a static window (frame + glass) in the selected profile sector.
