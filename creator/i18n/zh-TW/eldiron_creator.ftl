...
action_cut_hole_desc = 依據選取的內部區域在宿主區域上建立一個輪廓切口，並保留一個隱藏且已連結的幾何控制柄供後續建模使用。
...
action_build_shaft = 建立豎井
action_build_shaft_desc = 從隱藏的切口控制柄區域建立真正的豎井牆體，並可選擇建立底蓋。
action_build_stairs = 建立樓梯
action_build_stairs_desc = 在兩條選取的線段之間建立真正的直線樓梯。
...
action_window_desc = 在所選剖面區域建立靜態窗戶（窗框 + 玻璃）。
