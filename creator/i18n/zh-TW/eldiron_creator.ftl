...
status_action_edit_sector_terrain = 設定區域的地形角色。
...
status_action_edit_sector_item = 設定將為該區域實例化的物品。
status_action_edit_sector_visible = 區域幾何的可見狀態。
...
status_action_editing_slice_pos = 2D 編輯切片的垂直位置。
...
status_action_gate_door_inset = 門／門洞的內縮距離。
status_action_gate_door_tile = 門／門洞使用的圖塊。
status_action_gate_door_repeat_mode = 門／門洞的重複模式。
...
status_action_set_tile_material_roughness = 材質的粗糙度成分。
...
action_edit_sector_terrain_none = 無
...
action_edit_sector_item = 物品名稱
action_edit_sector_visible = 可見
