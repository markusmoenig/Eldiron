...
impl Draw2D {
    ...
    pub fn get_text_size(&self, font: &Font, size: f32, text: &str) -> (usize, usize) {
        ...
        let x = layout.glyphs()[layout.glyphs().len() - 1].x as usize
    ...
    /// Draws the given animated tile
    pub fn draw_animated_tile_with_blended_color(
        &self,
        frame: &mut [u8],
        pos: &(usize, usize),
        map: &TileMap,
    ...
    /// Draws the given animated tile
    pub fn draw_animated_tile_mult(
        &self,
        frame: &mut [u8],
        pos: &(usize, usize),
        map: &TileMap,
        ...
        for sy in 0..new_size.0 {
            ...
            for sx in 0..new_size.1 {
                ...
                let p = [
                    to_u8(to_float(pixels[s]) * to_float(mult_color[0])),
                    to_u8(to_float(pixels[s + 1]) * to_float(mult_color[1])),
                    to_u8(to_float(pixels[s + 2]) * to_float(mult_color[2])),
                    pixels[s + 3],
                ];
    ...
    /// Draw hover help
    pub fn create_buffer_for_message(
        &self,
        max_width: usize,
        font: &Font,
        font_size: f32,
        ...
        layout.append(
            fonts,
            &TextStyle::new(message.message.as_str(), font_size, 0),
        );
    ...
    /// Draw hover help
    pub fn create_buffer_for_multi_choice(
        &self,
        max_width: usize,
        font: &Font,
        font_size: f32,
        ...
        for glyph in layout.glyphs() {
            if glyph.char_data.rasterize() == false {
                continue;
            }
            let (metrics, alphamap) = font.rasterize(glyph.parent, glyph.key.px);
