    ...
    pub fn draw(&self, frame: &mut [u8], size: (usize, usize)) {
        ...
        for y in 0..height {
            for x in 0 .. width {
                ...
                for shape in &self.shapes {
                    ...
                    let d = match shape {
                        ScriptShapes::Rect(rect) => {
                            ...
                            let mut r : (f32, f32);
                            let rect = (rect.rect.0 as f32, rect.rect.1 as f32, rect.rect.2 as f32, rect.rect.3 as f32);

                            let p = (x as f32 - (rect.0 + rect.2 / 2.0), y as f32 - (rect.1 + rect.3 / 2.0));
                            ...
                            let q : (f32, f32) = (p.0.abs() - rect.2 / 2.0 + border_size + r.0, p.1.abs() - rect.3 / 2.0 + border_size + r.0);
                            f32::min(f32::max(q.0, q.1), 0.0) + length((f32::max(q.0, 0.0), f32::max(q.1, 0.0))) - r.0
                        },
                        ScriptShapes::RoundedRect(rect, round) => {

                            let rounding = (round.rect.3 as f32, round.rect.1 as f32, round.rect.2 as f32, round.rect.0 as f32);
                            let mut r : (f32, f32);
                            let rect = (rect.rect.0 as f32, rect.rect.1 as f32, rect.rect.2 as f32, rect.rect.3 as f32);
                ...
                let color = [(mixed_color[0] * 255.0) as u8, (mixed_color[1] * 255.0) as u8, (mixed_color[2] * 255.0) as u8, (mixed_color[3] * 255.0) as u8];
