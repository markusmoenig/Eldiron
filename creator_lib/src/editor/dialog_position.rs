...
impl DialogPositionWidget {
    pub fn new(_asset: &Asset, context: &ScreenContext) -> Self {
        ...
        let region_menu = AtomWidget::new(
            context.data.regions_names.clone(),
            AtomWidgetType::ToolBarMenuButton,
            AtomData::new("Regions", Value::Empty()),
        );
        widgets.push(region_menu);

        let areas_button = AtomWidget::new(
            vec!["Areas".to_string()],
            AtomWidgetType::ToolBarCheckButton,
            AtomData::new("Areas", Value::Empty()),
        );
        widgets.push(areas_button);

        let ok_button = AtomWidget::new(
            vec!["Accept".to_string()],
            AtomWidgetType::ToolBarButton,
            AtomData::new("Accept", Value::Empty()),
        );
        widgets.push(ok_button);

        let clear_button = AtomWidget::new(
            vec!["Clear".to_string()],
            AtomWidgetType::ToolBarButton,
    ...
    pub fn draw(
        ...
        if context.dialog_position_state == DialogState::Opening {
            ...
            self.dirty = true;
        } else if context.dialog_position_state == DialogState::Closing {
            context.dialog_height -= 20;
            rect.3 = context.dialog_height;
        ...
        if self.dirty {
            ...
            context.draw2d.draw_rounded_rect_with_border(
                ...
                &rect,
                rect.2,
                &(rect.2 as f32 - 1.0, rect.3 as f32 - 1.0),
                &context.color_black,
                &(20.0, 0.0, 20.0, 0.0),
            ...
            if context.dialog_position_state == DialogState::Open {
                ...
                context.draw2d.draw_rounded_rect_with_border(
                    ...
                    &region_rect,
                    rect.2,
                    &(region_rect.2 as f32 - 1.0, region_rect.3 as f32 - 1.0),
                    &context.color_black,
                    &(20.0, 20.0, 20.0, 20.0),
                    &border_color,
                    1.5,
                ...
                if context.data.regions_ids.is_empty() == false {
                    ...
                    if let Some(region) = context.data.regions.get(&region_id) {
                        ...
                        self.region_offset = context.draw2d.draw_region_centered_with_behavior(
                            buffer_frame,
                            region,
                            &region_rect,
                            &position,
                        ...
                        let mut oy: usize =
                            (region.data.max_pos.1 - region.data.min_pos.1) as usize;
                        ...
                        let o_rect = (
                            region_rect.0 + region_rect.2 - ox,
                            region_rect.1 + region_rect.3 - oy,
                            ox,
                            oy,
                        );
                        ...
                        if self.has_position {
                            dx = -(region.data.min_pos.0 + self.region_scroll_offset.0
                                - (region.data.min_pos.0 + position.0) / 2)
                                as isize
                                * size as isize;
                            dy = -(region.data.min_pos.1 + self.region_scroll_offset.1
                                - (region.data.min_pos.0 + position.1) / 2)
                                as isize
                                * size as isize;
                        } else {
                            dx = -(region.data.min_pos.0 + self.region_scroll_offset.0)
                        ...
                        context.draw2d.draw_rect_outline_safe(
                            buffer_frame,
                            &v_rect,
                            rect.2,
                            context.color_red,
        ...
        context.draw2d.blend_slice(
            frame,
            buffer_frame,
            &(self.rect.0, self.rect.1, rect.2, rect.3),
            context.width,
        );
            ...
            match key {
                ...
                WidgetKey::Return => {
                    ...
                    context.data.set_behavior_id_value(
                        context.dialog_node_behavior_id.clone(),
                        context.dialog_value.clone(),
    ...
    pub fn mouse_down(
        ...
        if context.contains_pos_for(local, self.overview_rect) {
            ...
            if let Some(region) = context.data.regions.get(&region_id) {
                ...
                if self.has_position == true {
                    self.region_scroll_offset.0 =
                        region.data.min_pos.0 + self.region_scroll_offset.0;
                    self.region_scroll_offset.1 =
                        region.data.min_pos.1 + self.region_scroll_offset.1;
                }
        ...
        for atom in &mut self.widgets {
            if atom.mouse_down(local, asset, context) {
                if atom.atom_data.id == "Clear" {
                    ...
                    return true;
                } else if atom.atom_data.id == "Regions" {
                    self.dirty = true;
                    self.clicked_id = atom.atom_data.id.clone();
                    return true;
                } else {
        ...
        if context.contains_pos_for(
            pos,
            (
                self.region_rect.0 + self.rect.0,
                self.region_rect.1 + self.rect.1,
                self.region_rect.2,
                self.region_rect.3,
            ),
        ) {
            ...
            let x = self.region_offset.0
                + ((cpos.0 - self.region_rect.0 - left_offset) / region_tile_size) as isize;
            let y = self.region_offset.1
                + ((cpos.1 - self.region_rect.1 - top_offset) / region_tile_size) as isize;
        ...
        for atom in &mut self.widgets {
            if atom.mouse_up(local, asset, context) {
                ...
                if self.clicked_id == "Clear" {
                    ...
                    self.new_value = true;
                } else if self.clicked_id == "Accept" {
                    context.dialog_position_state = DialogState::Closing;
                    context.target_fps = 60;
                    context.dialog_accepted = true;
