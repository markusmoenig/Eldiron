...
impl Widget for ToolBar {
        ...
        Self: Sized,
    {
        ...
        let mut item_button = AtomWidget::new(
            asset.tileset.maps_names.clone(),
            AtomWidgetType::ToolBarSliderButton,
            AtomData::new("Game", Value::Empty()),
        );
        item_button.set_rect((rect.0 + 10, rect.1, 200, rect.3));
        item_button.status_help_text =
            Some("Iterate over the items in the view. ('Tab').".to_string());
        widgets.push(item_button);

        let mut tiles_button = AtomWidget::new(
            vec!["Assets".to_string()],
            AtomWidgetType::ToolBarSwitchButton,
        ...
        tiles_button.status_help_text =
            Some("Edit the assets of your game ('1'). 'Space' to toggle.".to_string());
        widgets.push(tiles_button);

        let mut areas_button = AtomWidget::new(
        ...
        areas_button.status_help_text =
            Some("Edit the regions of your game ('2'). 'Space' to toggle.".to_string());
        widgets.push(areas_button);

        let mut behavior_button = AtomWidget::new(
            vec!["Characters".to_string()],
            AtomWidgetType::ToolBarSwitchButton,
            AtomData::new("Characters", Value::Empty()),
        );
        behavior_button.set_rect((rect.0 + 550, rect.1, 185, rect.3));
        behavior_button.status_help_text =
            Some("Edit the characters of your game ('3'). 'Space' to toggle.".to_string());
        behavior_button.custom_color = Some([47, 219, 37, 255]);

        widgets.push(behavior_button);

        let mut systems_button = AtomWidget::new(
        ...
        systems_button.status_help_text =
            Some("Edit the systems in your game ('4'). 'Space' to toggle.".to_string());
        widgets.push(systems_button);

        let mut items_button = AtomWidget::new(
            vec!["Items & Spells".to_string()],
            AtomWidgetType::ToolBarSwitchButton,
            AtomData::new("Items", Value::Empty()),
        );
        items_button.set_rect((rect.0 + 725 + 195, rect.1, 220, rect.3));
        items_button.status_help_text =
            Some("Edit the items and spells in your game ('5'). 'Space' to toggle.".to_string());
        items_button.custom_color = Some([205, 142, 67, 255]);

        widgets.push(items_button);

        let mut game_button = AtomWidget::new(
            vec!["Game".to_string()],
            AtomWidgetType::ToolBarSwitchButton,
            AtomData::new("Game", Value::Empty()),
        );
        ...
        game_button.status_help_text =
            Some("Edit the game logic and settings ('6'). '7' for game settings.".to_string());
    ...
    fn draw(
        ...
        context
            .draw2d
            .draw_rect(frame, &self.rect, context.width, &context.color_toolbar);
