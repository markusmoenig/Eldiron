...
impl NodePreviewWidget {
    ...
    pub fn draw(
        ...
        if self.dirty {
            ...
            context.draw2d.draw_rect(
                buffer_frame,
                &(rect.2 - 2, 0, 2, rect.3 - 1),
                stride,
                &context.color_black,
            );
            context
                .draw2d
                .draw_rect(buffer_frame, &(1, 1, 1, 1), stride, &[65, 65, 65, 255]);
            ...
            if self.graph_type == BehaviorType::Behaviors {
                if context.is_running {
                    ...
                    if let Some(position) = context.data.get_behavior_default_position(
                        context.data.behaviors_ids[context.curr_behavior_index],
                    ) {
            ...
            context.draw2d.blend_mask(
                buffer_frame,
                &(6, rect.3 - 23, rect.2, rect.3),
                rect.2,
                &context.preview_arc_mask[..],
                &(20, 20),
                &context.color_gray,
            );
        }
