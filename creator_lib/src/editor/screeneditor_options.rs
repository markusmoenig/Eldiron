        ...
        if let Some(content) = content {
            ...
            if let Some(tile) = content.get_selected_tile() {
                if let Some(map) = asset.get_map_of_id(tile.tilemap) {
                    context.draw2d.draw_animated_tile(frame, &((self.rect.2 - 100) / 2, self.rect.1 + 210), map, context.width, &(tile.x_off as usize, tile.y_off as usize), anim_counter, 100);
                }
                ...
                context.draw2d.draw_text_rect(frame, &(0, self.rect.1 + 310, self.rect.2, 20), context.width, &asset.get_editor_font("OpenSans"), 15.0, &format!("({}, {})", tile.x_off, tile.y_off), &context.color_white, &[0,0,0,255], crate::draw2d::TextAlignment::Center);
            }
        }
    }
            ...
            if atom.mouse_down(pos, asset, context) {
                ...
                //if mode == ScreenEditorMode::Scripts {
                    if atom.atom_data.id == "Scripts" {
                        ...
                        if let Some(content) = content {
                            content.set_current_script(self.widgets[1].groups[0].items[index].text.clone(), context);
                        }
                    }
                //}
