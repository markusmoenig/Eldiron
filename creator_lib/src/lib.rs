...
pub extern "C" fn rust_draw(pixels: *mut u8, width: u32, height: u32, anim_counter: usize) {
    ...
    EDITOR
        .lock()
        .unwrap()
        .draw(slice, width as usize, height as usize, anim_counter);
}
    ...
    if let Some(resource_path) = r_str.to_str().ok() {
        if let Some(projects_path) = p_str.to_str().ok() {
            EDITOR
                .lock()
                .unwrap()
                .init(resource_path.to_string(), projects_path.to_string());
        }
    }
}
...
pub extern "C" fn rust_touch_dragged(x: f32, y: f32) -> bool {
    ...
    EDITOR
        .lock()
        .unwrap()
        .mouse_dragged((x as usize, y as usize))
}
...
pub extern "C" fn rust_special_key_down(key: u32) -> bool {
    if key == KEY_ESCAPE {
        EDITOR
            .lock()
            .unwrap()
            .key_down(None, Some(WidgetKey::Escape))
    } else if key == KEY_RETURN {
        EDITOR
            .lock()
            .unwrap()
            .key_down(None, Some(WidgetKey::Return))
    } else if key == KEY_DELETE {
        EDITOR
            .lock()
            .unwrap()
            .key_down(None, Some(WidgetKey::Delete))
    } else if key == KEY_UP {
        ...
        EDITOR
            .lock()
            .unwrap()
            .key_down(None, Some(WidgetKey::Right))
        ...
        EDITOR
            .lock()
            .unwrap()
            .key_down(None, Some(WidgetKey::Space))
    } else {
        //if key == KEY_TAB {
...
pub extern "C" fn rust_key_modifier_changed(
    ...
    EDITOR
        .lock()
        .unwrap()
        .modifier_changed(shift, ctrl, alt, logo)
}
