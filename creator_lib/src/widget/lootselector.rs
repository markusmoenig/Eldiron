        ...
        for index in 0..max_tiles {
            ...
            if let Some(map) = asset.get_map_of_id(tile.tile.tilemap) {
                context.draw2d.draw_animated_tile(
                    ...
                    map,
                    stride,
                    &(tile.tile.x_off as usize, tile.tile.y_off as usize),
                    anim_counter,
                    self.grid_size,
                );

                if let Some(selected) = &self.selected {
                    if selected.tile.tilemap == map.settings.id
                        && selected.tile.x_off == tile.tile.x_off
                        && selected.tile.y_off == tile.tile.y_off
                    {
