...
impl TileMap {
    pub fn new(file_name: &PathBuf, base_path: &PathBuf) -> TileMap {
        ...
        let settings = serde_json::from_str(&contents).unwrap_or(TileMapSettings {
            grid_size: 16,
    ...
    pub fn get_tile(&self, tile_id: &(usize, usize)) -> Option<&Tile> {
        if let Some(tile) = self
            .settings
            .tiles
            .get(&(tile_id.0 as u16, tile_id.1 as u16))
        {
            Some(tile)
        } else {
            None
    ...
    pub fn get_mut_tile(&mut self, tile_id: &(usize, usize)) -> Option<&mut Tile> {
        if let Some(tile) = self
            .settings
            .tiles
            .get_mut(&(tile_id.0 as u16, tile_id.1 as u16))
        {
            Some(tile)
        } else {
            None
    ...
    pub fn set_tile(&mut self, tile_id: (usize, usize), tile: Tile) {
        self.settings
            .tiles
            .insert((tile_id.0 as u16, tile_id.1 as u16), tile);
    }
    ...
    pub fn offset_to_id(&self, offset: usize) -> (usize, usize) {
        (
            offset % (self.width / self.settings.grid_size),
            offset / (self.width / self.settings.grid_size),
        )
    }
}
...
pub fn update_tile_settings_sink(sink: &mut PropertySink) {
    if sink.contains("tags") == false {
        sink.properties
            .insert(0, Property::new_string("tags".to_string(), "".to_string()));
    }

    if sink.contains("raycaster_wall") == false {
        sink.properties.push(Property::new_string(
            "raycaster_wall".to_string(),
            "wall".to_string(),
        ));
    }
    ...
    if sink.contains("raycaster_sprite_shrink") == false {
        sink.properties
            .push(Property::new_int("raycaster_sprite_shrink".to_string(), 1));
    }

    if sink.contains("raycaster_sprite_move_y") == false {
        sink.properties.push(Property::new_float(
            "raycaster_sprite_move_y".to_string(),
            0.0,
        ));
    }
}
...
pub fn generate_tile_settings_sink_descriptions() -> FxHashMap<String, Vec<String>> {
    ...
    map.insert(
        "tags".to_string(),
        vec![
            "The comma separated tags for this tile, useful for searching for and grouping tiles"
                .to_string(),
        ],
    );
    map.insert(
        "raycaster_wall".to_string(),
        vec!["Display this tile in the raycaster as a \"wall\" or \"sprite\".".to_string()],
    );
    ...
    map.insert(
        "raycaster_sprite_move_y".to_string(),
        vec!["Moves the sprite up / down.".to_string()],
    );
