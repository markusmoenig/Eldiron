...
impl GameRegion {
    pub fn new(path: &PathBuf, region_path: &PathBuf) -> Self {
        ...
        let contents = fs::read_to_string(level1_path).unwrap_or("".to_string());

        let mut data = serde_json::from_str(&contents).unwrap_or(GameRegionData {
        ...
        let procedural = Some(GameBehavior::load_from_path(
            &procedural_path,
            &procedural_path,
        ));
    ...
    pub fn save_data(&self) {
        ...
        if let Some(json) = serde_json::to_string(&self.data).ok() {
            fs::write(json_path, json).expect("Unable to write region file");
        }
    }
    ...
    pub fn get_center_offset_for_visible_size(
        &self,
        visible_tiles: (usize, usize),
    ) -> (isize, isize) {
        let x = self.data.min_pos.0 + (self.data.max_pos.0 - self.data.min_pos.0) / 2
            - visible_tiles.0 as isize / 2;
        let y = self.data.min_pos.1 + (self.data.max_pos.1 - self.data.min_pos.1) / 2
            - visible_tiles.1 as isize / 2;
        (x, y)
    }
    ...
    pub fn remap(&mut self, asset: &mut Asset) {
        ...
        for id in &ids {
            ...
            if let Some(tile) =
                asset.get_tile(&TileId::new(value.tilemap, value.x_off, value.y_off))
            {
                tiles.insert(
                    **id,
                    TileData {
...
pub fn update_region_sink(sink: &mut PropertySink) {
    if sink.contains("background") == false {
        sink.properties.insert(
            0,
            Property::new_color("background".to_string(), "#000000".to_string()),
        );
    }
    ...
    if sink.contains("movement") == false {
        sink.properties.insert(
            1,
            Property::new_string("movement".to_string(), "tile".to_string()),
        );
    }
...
pub fn generate_region_sink_descriptions() -> FxHashMap<String, Vec<String>> {
    ...
    map.insert(
        ...
        vec!["The background color of the region".to_string()],
    );
    map.insert(
        "movement".to_string(),
        vec![
            "Use \"tile\" for tile based movement or \"pixel\" for sub-tile movement.".to_string(),
        ],
    );
    ...
    map.insert(
        "base_lighting".to_string(),
        vec![
            "The base lighting of the region. 0.0 for fully black and 1.0 for fully lit."
                .to_string(),
        ],
    );
    map.insert(
        "visibility".to_string(),
        vec![
            "Use \"full\" for unlimited visibility or \"limited\" to enable the parameters below."
                .to_string(),
        ],
    );
    map.insert(
        "visible_distance".to_string(),
        vec![
            "The visible distance in tiles. \"visibility\" has to be set to \"limited\"."
                .to_string(),
        ],
    );
    map.insert(
        "supports_3d".to_string(),
        vec!["Does this region support 3D ?".to_string()],
    );
