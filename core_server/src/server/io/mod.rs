...
pub trait ServerIO: Sync + Send {
    ...
    fn system_login(&mut self, url: String, password: String) -> Result<bool, IOError> {
        Err(NotImplemented)
    }
    ...
    /// Does the user exist ?
    fn does_user_exist(&mut self, user_name: String) -> Result<bool, IOError> {
        Err(NotImplemented)
    }
    ...
    fn get_user_character(
        &self,
        user_name: String,
        character_name: String,
    ) -> Result<Sheet, IOError> {
        Err(NotImplemented)
    }
    ...
    fn error_message(&self, err: IOError) -> Option<String> {
        match err {
            UserNotFound => Some("User not found".into()),
            UserAlreadyExists => Some("User already exists".into()),
            WrongPassword => Some("Wrong password".into()),
            _ => None,
        }
    }
}
