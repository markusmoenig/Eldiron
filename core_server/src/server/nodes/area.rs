    ...
    if let Some(characters) = data.characters.get(&id.0) {
        for character_data in characters {
            if let Some(position) = data.get_instance_position(character_data.index) {
                if data.region_data.areas[data.curr_area_index]
                    .area
                    .contains(&(position.x, position.y))
                {
                    if data.area_characters.contains_key(&data.curr_area_index) == false {
                        data.area_characters
                            .insert(data.curr_area_index, vec![character_data.index]);
                    } else if let Some(area_list) =
                        data.area_characters.get_mut(&data.curr_area_index)
                    {
                    ...
                    if was_inside_already == false {
                        if enter_everyone {
                            ...
                            .prev_area_characters
                            .contains_key(&data.curr_area_index)
                            == false
                        {
...
pub fn node_message_area(
    ...
    if let Some(action_index) = data.curr_action_character_index {
        ...
        data.character_instances[action_index]
            .messages
    ...
    if let Some(area_list) = data.area_characters.get(&data.curr_area_index) {
        ...
        for index in area_list {
            data.character_instances[*index]
                .messages
                .push(message_data.clone());
        }
    }
...
pub fn node_light_area(
    ...
    for pos in &region.areas[data.curr_area_index].area {
        let light = LightData::new(
            core_shared::lightdata::LightType::PointLight,
            (pos.0, pos.1),
            1,
        );
