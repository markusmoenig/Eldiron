...
use tilegraph::{
    RenderedTileGraph, TileGraphDocument, TileGraphRenderer, TileGraphSubgraphResolver,
    TileNodeGraphExchange, TileNodeGraphState, flatten_graph_exchange_with,
};

struct FileLayerResolver {
    base_dir: PathBuf,
}

impl FileLayerResolver {
    fn resolve_path(&self, source: &str) -> Option<PathBuf> {
        let source = source.trim();
        if source.is_empty() {
            return None;
        }
        let direct = self.base_dir.join(source);
        if direct.exists() {
            return Some(direct);
        }
        let tilegraph = self.base_dir.join(format!("{source}.tilegraph"));
        if tilegraph.exists() {
            return Some(tilegraph);
        }
        None
    }
}

impl TileGraphSubgraphResolver for FileLayerResolver {
    fn resolve_subgraph_state(&self, source: &str) -> Option<TileNodeGraphState> {
        self.resolve_subgraph_exchange(source)
            .map(|exchange| exchange.graph_state)
    }

    fn resolve_subgraph_exchange(&self, source: &str) -> Option<TileNodeGraphExchange> {
        let path = self.resolve_path(source)?;
        let text = fs::read_to_string(&path).ok()?;
        let mut exchange = load_graph_from_text(&path, &text).ok()?;
...
fn print_usage() {
    eprintln!(
        "Usage: tilegraph <graph.eldiron_graph|graph.tilegraph> [output_dir] [--palette #RRGGBB,#RRGGBB,...]"
    );
}
...
fn run() -> Result<(), String> {
    ...
    let resolver = FileLayerResolver {
        base_dir: input_path
            .parent()
            .map(Path::to_path_buf)
            .unwrap_or_else(|| PathBuf::from(".")),
    };
    graph = flatten_graph_exchange_with(&graph, &resolver);
