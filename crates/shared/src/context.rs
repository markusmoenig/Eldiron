...
impl ProjectContext {
    ...
    pub fn is_screen(&self) -> bool {
        match self {
            ...
            | ProjectContext::ScreenWidget(_, _) => true,
            _ => false,
        }
    }
    ...
    pub fn is_game_shortcuts(&self) -> bool {
        match self {
            ProjectContext::GameShortcuts => true,
            _ => false,
        }
    }
...
impl ServerContext {
    ...
    pub fn snap_scalar(value: f32, subdivisions: f32) -> f32 {
        let step = Self::edit_grid_step(subdivisions);
        (value / step).round() * step
    }

    pub fn edit_grid_step(subdivisions: f32) -> f32 {
        let subdivisions = subdivisions.round().clamp(1.0, 32.0);
        let power = subdivisions.log2().round().clamp(0.0, 5.0);
        1.0 / 2.0_f32.powf(power)
    }
...
mod tests {
    ...
    fn edit_grid_step_rounds_and_clamps_subdivisions() {
        ...
        assert_eq!(ServerContext::edit_grid_step(16.0), 0.0625);
        assert_eq!(ServerContext::edit_grid_step(32.0), 0.03125);
