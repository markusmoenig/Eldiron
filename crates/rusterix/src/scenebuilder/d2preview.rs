...
impl D2PreviewBuilder {
    ...
    pub fn set_properties(&mut self, properties: &ValueContainer) {
        ...
        self.editing_slice_height = properties.get_float_default("editing_slice_height", 2.0);
    }
    ...
    pub fn build_entities_items(
        ...
        if draw_sectors {
            ...
            for sector in &sectors {
                if sector.intersects_vertical_slice(
                    map,
                    self.editing_slice,
                    self.editing_slice_height,
                ) {
                    let bbox = sector.bounding_box(map);
            ...
            || self.map_tool_type == MapToolType::Rect
        {
            ...
            for linedef in &map.linedefs {
                if !linedef.intersects_vertical_slice(
                    map,
                    self.editing_slice,
                    self.editing_slice_height,
                ) {
